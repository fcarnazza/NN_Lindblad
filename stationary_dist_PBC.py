...
for pot in range(len(conf['potential'])):
 for inv_T in range(len(conf['beta'])):
   ...
   Lindblad = get_L_from_model( './trained_models/PBC/Thermal/eff__Cdef_%s_beta_%s_L_%s_two_bodies_LL' %(C_def, beta,L)  )
   Lambdas, v_s = LA.eig(Lindblad)
   v_stat = v_s[-1]
   gap = -np.sort(np.real(Lambdas))[-2]
   ...
   with open("stationary_diff_{sub_n}_OBC.txt", "a") as file_object:
    file_object.write("pot %f beta %f  d_trace %f\n"%( C_def,  beta, d_trace )) #Matr[pot,inv_T] = tr_d/nstep
