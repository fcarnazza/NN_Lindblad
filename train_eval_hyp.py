...
if __name__ == '__main__':
    ...
    for k in range(len(cfg["beta"])):
        for j in range(len(cfg["potential"])):
            ...
            data_train = Data_p_t('./results/data_th_periodic/T_10/traj_fss_dyn_%s_%s_.h5' %(beta, C))
            data_eval = Data_p_t('./results/data_th_periodic/T_10/traj_fss_dyn_%s_%s_eval.h5' %(beta, C))
            ...
            torch.save(model.state_dict(), './trained_models/PBC/hyper/eff_Cdef_%s_beta_%s_hyper_' %(C, beta))
