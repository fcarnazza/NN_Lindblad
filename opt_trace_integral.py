 ...
 for inv_T in range(len(conf['beta'])):
   ...
   for i in range( 1, nstep ):
      ...
      with torch.no_grad():
        ...
        diff += np.sum(np.abs(r.reshape(16)-rho_exact.reshape(16) ) )      
   ...
   with open("diff_sub_n_spin_chain_final.txt", "a") as file_object:
     file_object.write(" %d %d pot %f beta %f tr_d/nstep %f\n"%(sub_it_c, sub_it_b, C_def,  beta, diff)) #Matr[pot,inv_T] = tr_d/nstep
"""
for i in range(15):
    fig = plt.figure()
