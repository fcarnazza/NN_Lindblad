...
def compute_traj2(site, beta, dt, T):
    ...
    rho1 = np.diag([np.random.normal() for i in range(2)])
    rho2 = np.diag([np.random.normal() for i in range(2)])
    rho_loc = np.kron(rho1/rho1.trace(),rho2/rho2.trace()) 
...
if __name__ == '__main__':
    ...
    for k in range(len(cfg["system"]["potential"])):
        for j in range(len(cfg["system"]["beta"])):
            ...
            path = os.path.join(cfg["model_dir"], f"traj_fss_dyn_{beta}_{C_def}_eval_2_spin.h5")
            path_eval_fin = os.path.join(cfg["model_dir"], f"traj_fss_final_{beta}_{C_def}_eval_2_spin.h5")
