...
if __name__ == '__main__':
    ...
    for k in range(len(cfg["alpha"])):
        for j in range(len(cfg["potential"])):
            alpha = cfg["alpha"][k]
            C = cfg["potential"][j]
            L = cfg["n_spins"]
            dt = cfg["MLP_pr"]["mlp_params"]["dt"]
            print('===Training model for alpha = %s and C = %s' %(alpha, C))
            #data_train = Data_p_train("./results/data_th_open/T_10/traj_%s_%s.h5" %(alpha, C))
            #data_eval = Data_p_eval("./results/data_th_open/T_10/traj_%s_%s.h5" %(alpha, C))
            data_train = Data_p_train("./results/data_th_open/T_10/traj_alpha_%s_C_%s_n_spin_%s_dt_%s.h5"%(alpha,C,L,dt))
            data_eval = Data_p_eval("./results/data_th_open/T_10/traj_alpha_%s_C_%s_n_spin_%s_dt_%s.h5"%(alpha,C,L,dt))
            ...
            torch.save(model.state_dict(), './trained_models/OBC/Thermal/eff__alpha_%s_C_%s_n_spins_%s_dt_%s' %(alpha, C,L,dt))
