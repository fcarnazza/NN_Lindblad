...
if __name__ == '__main__':
    ...
    for j in range(len(cfg["potential"])):
            C = cfg["potential"][j]
            print('===Training model and C = %s' %(C))
            #data_train = Data_p_train("./results/data_th_periodic/T_10/traj_%s_%s.h5" %(beta, C))
            data_train = Data_p_train("./results/data_th_periodic/T_10/two_bodies/traj_fss_dyn_%s_Lindblad_diag.h5" %( C))
            data_eval = Data_p_eval("./results/data_th_periodic/T_10/two_bodies/traj_fss_dyn_%s_Lindblad_diag.h5" %(C))
            ...
            torch.save(model.state_dict(), './trained_models/PBC/Thermal/eff__Cdef_%s_two_bodies_Lindblad' %(C))
