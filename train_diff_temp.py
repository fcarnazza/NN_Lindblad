...
if __name__ == '__main__':
    ...
    for j in range(len(cfg["potential"])):
            C_def = cfg["potential"][j]
            beta1 = cfg["beta1"]
            beta2 = cfg["beta2"]
            print('===Training model for C = %s' %( C_def))
            data_train = Data_p_train("./results/data_th_open/T_10/diff_temp/traj_fss_dyn_%s_%s_%s.h5" %(beta1, beta2, C_def))
            data_eval = Data_p_eval("./results/data_th_open/T_10/diff_temp/traj_fss_dyn_%s_%s_%s.h5" %(beta1, beta2, C_def))
            #data_f = Data_p_final('./results/data_th_periodic/T_20/traj_fss_final_0_0.9.h5')
            ...
            model = MLP(**cfg.MLP_pr).to(cfg.device)
            #model = BVAE(**cfg.BVAE_params).to(cfg.device)
            ...
            torch.save(model.state_dict(), './trained_models/OBC/Thermal/eff_beta1_%s_beta2_%s_C_%s' %(beta1, beta2, C_def))
